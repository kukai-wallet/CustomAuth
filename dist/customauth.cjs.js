/******/ (() => {
  ...
  const CONTRACT_MAP = {
    [TORUS_NETWORK.MAINNET]: fetch_node_details_default().PROXY_ADDRESS_MAINNET,
    [TORUS_NETWORK.TESTNET]: fetch_node_details_default().PROXY_ADDRESS_ROPSTEN,
    [TORUS_NETWORK.CYAN]: fetch_node_details_default().PROXY_ADDRESS_POLYGON,
  };
  const NETWORK_MAP = {
    [TORUS_NETWORK.MAINNET]: fetch_node_details_namespaceObject.ETHEREUM_NETWORK.MAINNET,
    [TORUS_NETWORK.TESTNET]: fetch_node_details_namespaceObject.ETHEREUM_NETWORK.ROPSTEN,
    [TORUS_NETWORK.CYAN]: fetch_node_details_namespaceObject.ETHEREUM_NETWORK.POLYGON,
  };
  const SIGNER_MAP = {
  ...
  function getPopupFeatures() {
    ...
    const features = "titlebar=0,toolbar=0,status=0,location=0,menubar=0,height="
  ...
  function are3PCSupported() {
    ...
    let thirdPartyCookieSupport = true; // brave
      ...
      if (this.uxMode === UX_MODE.REDIRECT) {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (!this.redirectToOpener) {
            bc = new broadcast_channel_namespaceObject.BroadcastChannel("redirect_channel_".concat(this.nonce), broadcastChannelOptions);
    ...
    async getUserInfo(params) {
      ...
      if (accessToken) {
        try {
          ...
          const userInfo = await (0, http_helpers_namespaceObject.get)("".concat(padUrlString(domainUrl)).concat(user_info_route), {
            headers: {
              Authorization: "Bearer ".concat(accessToken),
            },
          });
  ...
  class PasswordlessHandler_JwtHandler extends handlers_AbstractLoginHandler {
    ...
    async getUserInfo(params) {
      ...
      try {
        const domainUrl = new URL(domain);
        const userInfo = await (0, http_helpers_namespaceObject.get)("".concat(padUrlString(domainUrl), "userinfo"), {
          headers: {
            Authorization: "Bearer ".concat(accessToken),
          },
        });
    ...
    handleLoginWindow() {
      return new Promise((resolve, reject) => {
        ...
        const bc = new broadcast_channel_namespaceObject.BroadcastChannel("redirect_channel_".concat(this.nonce), broadcastChannelOptions);
        bc.addEventListener("message", async (ev) => {
          handleData(ev);
          bc.close();
        });
      ...
      if (extraParamsPassed === "true") {
        ...
        try {
          ...
          ({
            ...
            rpOrigin,
            credId,
          } = await (0, http_helpers_namespaceObject.get)("".concat(WEBAUTHN_LOOKUP_SERVER, "/signature/fetch/").concat(idToken)));
        }
      } else {
  ...
  class StorageHelper {
    ...
    async storeLoginDetails(params, scope) {
      ...
      if (this.currentStorageMethod === REDIRECT_PARAMS_STORAGE_METHOD.SERVER) {
        ...
        const signature = (await (0, eccrypto_namespaceObject.sign)(privKey, (0, metadata_helpers_namespaceObject.keccak256)(encData))).toString(
          "hex"
        );
        await (0, http_helpers_namespaceObject.post)("".concat(this.storageServerUrl, "/store/set"), {
          key: publicKeyHex,
          data: encData,
          signature,
        });
    ...
    async retrieveLoginDetails(scope) {
      ...
      if (this.currentStorageMethod === REDIRECT_PARAMS_STORAGE_METHOD.SERVER) {
        ...
        const privKeyHex = privKey.toString("hex");
        const publicKeyHex = (0, eccrypto_namespaceObject.getPublic)(privKey).toString("hex");
        const encData = await (0, http_helpers_namespaceObject.get)("".concat(this.storageServerUrl, "/store/get?key=").concat(publicKeyHex));
        const loginDetails = await (0, metadata_helpers_namespaceObject.decryptData)(privKeyHex, encData.message);
        return loginDetails;
      }

      const loginDetails = window.localStorage.getItem("torus_login_".concat(scope));
  ...
  class CustomAuth {
    ...
    async triggerLogin(args) {
      ...
      if (checkIfNewKey || skipTorusKey === SkipTorusKey.IfNew) {
        ...
        const lookupData = await (0, torus_js_namespaceObject.keyLookup)(torusNodeEndpoints, verifier, userInfo.verifierId);
        existingPk =
          lookupData !== null &&
          lookupData !== void 0 &&
    ...
    async triggerAggregateLogin(args) {
      ...
      if (checkIfNewKey || skipTorusKey === SkipTorusKey.IfNew) {
        ...
        const lookupData = await (0, torus_js_namespaceObject.keyLookup)(torusNodeEndpoints, args.verifierIdentifier, userInfoData[0].verifierId);
        existingPk =
          lookupData !== null &&
          lookupData !== void 0 &&
    ...
    async triggerHybridAggregateLogin(args) {
      const { singleLogin, aggregateLoginParams } = args; // This method shall break if any of the promises fail. This behaviour is intended
