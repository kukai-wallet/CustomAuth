...
class StorageHelper {
  ...
  async storeLoginDetails(params, scope) {
    ...
    if (this.currentStorageMethod === REDIRECT_PARAMS_STORAGE_METHOD.SERVER) {
      ...
      await post("".concat(this.storageServerUrl, "/store/set"), {
        key: publicKeyHex,
        data: encData,
        signature,
      });
  ...
  async retrieveLoginDetails(scope) {
    ...
    if (this.currentStorageMethod === REDIRECT_PARAMS_STORAGE_METHOD.SERVER) {
      ...
      const privKeyHex = privKey.toString("hex");
      const publicKeyHex = getPublic(privKey).toString("hex");
      const encData = await get("".concat(this.storageServerUrl, "/store/get?key=").concat(publicKeyHex));
...
class CustomAuth {
  ...
  async triggerAggregateLogin(args) {
    ...
    const aggregateIdToken = keccak256$1(aggregateIdTokenSeeds.join(String.fromCharCode(29))).slice(2);
