...
import Torus, { legacyKeyLookup, keccak256 as keccak256$1 } from "@toruslabs/torus.js";
...
class CustomAuth {
  ...
  async triggerLogin(args) {
    ...
    if (registerOnly) {
      ...
      return _objectSpread(
        _objectSpread(_objectSpread({}, res), torusPubKey),
        {},
        {
  ...
  async triggerAggregateLogin(args) {
    ...
    const userInfoData = userInfoArray.map((x, index) => _objectSpread(_objectSpread({}, x), loginParamsArray[index]));
    let skip = true;
    let existingPk;
    if (checkIfNewKey || skipTorusKey === SkipTorusKey.IfNew) {
  ...
  async triggerHybridAggregateLogin(args) {
    ...
    if (hash && queryParameters) {
      ...
      if (error) throw new Error(error);
      const { access_token: accessToken, id_token: idToken } = hashParameters,
        rest = _objectWithoutProperties(hashParameters, _excluded3);
      // State has to be last here otherwise it will be overwritten
      loginParams = _objectSpread(
        _objectSpread(
          {
  ...
  async getRedirectResult() {
    ...
    const _await$this$storageHe = await this.storageHelper.retrieveLoginDetails(instanceId),
      ...
      rest = _objectWithoutProperties(_await$this$storageHe, _excluded4);
